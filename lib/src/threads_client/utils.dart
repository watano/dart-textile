...
void handleListenData(ListenReply data, EventSink<ListenResult> sink) {
  final result = ListenResult.fromJson({'collectionName': data.collectionName, 'instanceID': data.instanceID, 'action': data.action.name, 'instance': JsonCodec().decode(utf8.decode(data.instance))});
  sink.add(result);
}
