...
class Client {
  ...
  Client({Config config}) {
    ...
    _host = config.host;
    _port = config.port;
    _channel = ClientChannel(_host, port: _port, options: ChannelOptions(credentials: config.credentials));
    _stub = APIClient(_channel, options: config.getCallOptions());
  }
  ...
  Future<void> newDBFromAddr({String address, String key, List<Collection> collections}) async {
    ...
    request.collections.addAll(collections.map((c) => c.getConfig()));
  ...
  Future<Info> getDBInfo(String dbID) async {
    ...
    final response = Info(output.addrs.map((addr) => base64.encode(addr)).toList(), base64.encode(output.key));
    return response;
  }
  ...
  Future<void> delete(String dbID, String collectionName, List<String> instanceIDs) async {
    ...
    request.dbID = base64.decode(dbID);
    request.collectionName = collectionName;
    request.instanceIDs.addAll(instanceIDs);
  ...
  ResponseStream<GetTokenReply> getToken(String key, String signature) {
    final request = GetTokenRequest();
    request.key = key;
    request.signature = base64.decode(signature);
    final controller = StreamController<GetTokenRequest>();
    controller.sink.add(request);
    final response = _stub.getToken(controller.stream);
    controller.close();
    return response;
  }
