...
void main() async {
  ...
  setUpAll(() {
    ...
    final config = threads.Config(
      host: env['THREADS_HOST'] ?? '127.0.0.1',
      port: int.parse(env['THREADS_PORT'] ?? '6006'),
    );
...
class Person {
  ...
  Map<String, dynamic> toJson() => {'_id': id, 'firstName': firstName, 'lastName': lastName, 'age': age, 'biker': biker};
}
